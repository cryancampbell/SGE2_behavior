...
for (rowNum in 1:dim(dataCombos)[1]) {
  ...
  if (adjustData == FALSE) {
    samplingDate <- "WholePhase"
  }
  
  # R of order of intro ~ order of Elo
  rIntroElo <- cor.test(sge.behave.summary$calcOrdRank, sge.behave.summary$intro.no)$estimate
  # p of order of intro ~ order of Elo
  pIntroElo <- cor.test(sge.behave.summary$calcOrdRank, sge.behave.summary$intro.no)$p.value
  ...
  ggplot(data = sge.behave.summary,
    ...
    labs(col = "Group") +
    xlab("Order of Introduction") + ylab("Ordinal Rank from Elo") +
    ggtitle(paste0("Ordinal Rank v. Introduction Order, ",samplingDate," SGEII Phase ",sgePhase),
            subtitle = paste0("R = ",round(rIntroElo, digits = 3)))
  ggsave(paste0(figDir,"sge2phase",sgePhase,"_",samplingDate,"_EloRank_v_IntroOrder_XYplot.png"),
         width = 6, height = 4)
  ...
  write.table(x = sge.behave.summary.full,
              file = paste0(outDir,"sge2Behavior_phase",sgePhase,"_adj",samplingDate,".txt"),
              quote = FALSE, 
              sep = ",")
  write.table(x = sge.behave.summary,
              file = paste0(outDir,"sge2Behavior_shortCols_phase",sgePhase,"_adj",samplingDate,".txt"),
              quote = FALSE, 
              sep = ",")
  
  methodsOut <- file(paste0(outDir,"sge2Behavior_phase",sgePhase,"_adj",samplingDate,"_methods.txt"))
...
for (samplingDate in c("SingleCell","Vax","EnvChall")) {
  ...
  ggsave(paste0(figDir,"sge2phase1_",samplingDate,"vFull_centElo_XYplot.png"),
         width = 6, height = 4)
  ...
  ggsave(paste0(figDir,"sge2phase1_",samplingDate,"vFull_centAgRec_XYplot.png"),
         width = 6, height = 4)
  ...
  ggplot(data = compareData,
    ...
    ggsave(paste0(figDir,"sge2phase1_",samplingDate,"vFull_centGroom_XYplot.png"),
         width = 6, height = 4)
