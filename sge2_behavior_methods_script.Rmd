...
samplingDates <- read.table("~/Documents/giterdone/SGE2_behavior/data/samplingDates_allSGE2_forR.csv", sep = ",", header = T)
...
for (a in allAnimals) {
  #get group
  grpID <- samplingDates$groupP1[which(samplingDates$animal == a)]
  #use april 1st as plot end date
  plotEndDate <- as.Date("2020-04-01")
  absentDates <- vector()
  ...
  if ( !is.na(scDate) ) {
    ...
    groupVector <- c(groupVector, groupID)
    eventVector <- c(eventVector, "singleCell")
    dateVector <- c(dateVector, scDate)
  }
  ecDate <- samplingDates$dateEnvChallP1[which(samplingDates$animal == a)]
  if ( !is.na(ecDate) ) {
    animalVector <- c(animalVector, a)
    groupVector <- c(groupVector, groupID)
  ...
  vaxDate <- samplingDates$dateVaxP1[which(samplingDates$animal == a)]
  if ( !is.na(vaxDate) ) {
    animalVector <- c(animalVector, a)
    groupVector <- c(groupVector, groupID)
  ...
  groupCompleteDate <- subset(clinicalPulls, phase == "SGE1" & groupID == grpID)$group_completion_date[1]
  
  #if the animal was pulled, do these things:
  if ( a %in% subset(clinicalPulls, phase == "SGE1")$Animal ) {
    animalPulls <- subset(clinicalPulls, Animal == a & phase == "SGE1")
...
groupDates <- unique(subset(clinicalPulls, phase == "SGE1")[,2:3])
...
sge2plotDates <- subset(sge2dates, date < "2019-04-01")
...
samplingDate <- "Vax" #or Vax or EnvChall
sgePhase <- 2
rowNum <- 3
...
for (rowNum in 1:dim(dataCombos)[1]) {
  ...
  if (sgePhase == 1) {
    ...
    dateDF <- rbind(dateDF,c("Sd4","SPF1"))
  }
  ...
  ### This code allows for the dates to be modified
  ...
  sge.behave.summary$groupStart <- NA
  ...
  for (i in sge.behave.summary$ID) {
    ...
    if (i %in% dateDF$twoLetter) {
      ...
      if (subset(clinicInfo, Phase == paste0("SGE",sgePhase) & Animal == sge.behave.summary$animalID[behaveRow])$Death == "yes") {
        sge.behave.summary$diedInPhase[behaveRow] <- TRUE
      }
      
      sge.behave.summary$daysOut[behaveRow] <- subset(clinicInfo, Phase == paste0("SGE",sgePhase) & Animal == sge.behave.summary$animalID[behaveRow])$DaysOutTot
  ...
  sge.behave.summary$groupStart <- as.Date(sge.behave.summary$groupStart)
  ...
  #!#!# Need some code here to determine if animals were in/out of group long enough #!#!#
  
  #this line currently calculates "days in clinic" (daysOut from file) over total group days between group start and experiment
  sge.behave.summary$daysOut / as.numeric(sge.behave.summary$grpExpDate - sge.behave.summary$groupStart) > .5
  #individuals < 0.5 for this metric are guaranteed to have been in the group over half the time
  #individuals > 0.5 i need to check WHEN the dates they missed were, so i need to import more thorough data
